...
export class WebSocketStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const websocketApi = new apigw2.WebSocketApi(this, "WSApi", {
        apiName: "WSApi",
        connectRouteOptions: {
            integration: new apigw2Integrations.WebSocketLambdaIntegration("ConnectionHandler", connectionHandler)
        },
        disconnectRouteOptions: {
            integration: new apigw2Integrations.WebSocketLambdaIntegration("DisconnectionHandler", disconnectionHandler)
        }
    });
    ...
    const receiverHandler = new lambdaNodeJS.NodejsFunction(this,
        "ReceiverHandler", {
            runtime: lambda.Runtime.NODEJS_20_X,
    ...
    websocketApi.addRoute('action1', {
        integration: new apigw2Integrations.WebSocketLambdaIntegration("ReceiverHandler", receiverHandler)
    });

    // Lambda para WebSocket response
    const executorHandler = new lambdaNodeJS.NodejsFunction(this,
        "ExecutorHandler", {
            runtime: lambda.Runtime.NODEJS_20_X,
    ...
    executorHandler.addEventSource(new lambdaEventSources.SqsEventSource(actionSqs, {
        batchSize: 5,
